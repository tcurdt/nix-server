      ...
      serviceConfig = {
        ExecStartPost =
          let
            ipv4Cmd = optional (
              cfg.ipv4 != null
            ) "${pkgs.iproute2}/bin/ip addr add ${cfg.ipv4} dev ${cfg.interface}";
            ipv4RouteCmd = optional (
              cfg.ipv4Gateway != null
            ) "${pkgs.iproute2}/bin/ip route add default via ${cfg.ipv4Gateway} dev ${cfg.interface}";
            ipv6Cmd = optional (
              cfg.ipv6 != null
            ) "${pkgs.iproute2}/bin/ip addr add ${cfg.ipv6} dev ${cfg.interface}";
            ipv6RouteCmd = optional (
              cfg.ipv6Gateway != null
            ) "${pkgs.iproute2}/bin/ip -6 route add default via ${cfg.ipv6Gateway} dev ${cfg.interface}";
          in
        ...
        ExecStop =
          let
            ...
            ipv6Cmd = optional (
              cfg.ipv6 != null
            ) "${pkgs.iproute2}/bin/ip addr del ${cfg.ipv6} dev ${cfg.interface}";
            ipv6RouteCmd = optional (
              cfg.ipv6Gateway != null
